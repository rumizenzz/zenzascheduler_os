...
Deno.serve(async (req) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for grace-prayers" ON storage.objects FOR SELECT USING (bucket_id = 'grace-prayers');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for grace-prayers" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'grace-prayers');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for grace-prayers" ON storage.objects FOR UPDATE USING (bucket_id = 'grace-prayers');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for grace-prayers" ON storage.objects FOR DELETE USING (bucket_id = 'grace-prayers');`
    ];
