...
export function BrowserOSModule() {
  ...
  type AppType = 'note' | 'files' | 'terminal' | 'settings'

  const createWindow = async (app: AppType) => {
    if (!user) return
    const defaults: Record<AppType, { title: string; content: string }> = {
      note: { title: 'New Note', content: '' },
      files: { title: 'File Explorer', content: JSON.stringify({ files: [] }) },
      terminal: { title: 'Terminal', content: '' },
      settings: {
        title: 'Settings',
        content: JSON.stringify({ darkMode: true }),
      },
    }
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="w-full h-[600px] border border-purple-300/30 rounded-md bg-purple-900/20 flex flex-col">
