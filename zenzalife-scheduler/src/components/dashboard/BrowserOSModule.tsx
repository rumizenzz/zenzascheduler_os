...
export function BrowserOSModule() {
  ...
  const finishRename = (id: string) => {
    const name = renameValue.trim() || 'New Folder'
    setFolders((fs) =>
  ...
  const handleFolderClick = (folder: OSDesktopItem) => {
  ...
  const handleFolderDoubleClick = (folder: OSDesktopItem) => {
    if (clickTimeout.current) {
      clearTimeout(clickTimeout.current)
      clickTimeout.current = null
    }
  ...
  const handleDesktopMouseMove = (
    ...
    const x = Math.min(currX, selectionStart.x)
    const y = Math.min(currY, selectionStart.y)
            ...
            >
              <div
                className={`flex flex-col items-center text-purple-200 cursor-pointer ${
                  selectedIds.includes(f.id) ? 'bg-purple-700/40 rounded' : ''
                }`}
                onClick={() => handleFolderClick(f)}
                onDoubleClick={(e) => {
                  e.stopPropagation()
                  handleFolderDoubleClick(f)
                }}
              >
                <Folder className="w-10 h-10" />
                {renamingId === f.id ? (
                  <input
                    className="mt-1 w-full text-xs bg-purple-900/40 text-center focus:outline-none rounded"
                    value={renameValue}
                    autoFocus
                    onChange={(e) => setRenameValue(e.target.value)}
                    onBlur={() => finishRename(f.id)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') finishRename(f.id)
                    }}
                  />
                ) : (
