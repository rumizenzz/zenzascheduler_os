...
export function MathNotebookModule() {
    ...
    const load = async () => {
      ...
      if (data && data.length > 0) {
        ...
        const newTab: TabData = {
          id: inserted.id,
          name: inserted.name,
          data: inserted.data || { elements: [], appState: {} },
          history: []
        }
        setTabs([newTab])
  ...
  const addTab = async () => {
    ...
    const newTab: TabData = {
      id: data.id,
      name: data.name,
      data: data.data || { elements: [], appState: {} },
      history: []
    }
    setTabs([...tabs, newTab])
    setActiveTabId(newTab.id)
  ...
  const updateTabData = async (elements: unknown, appState: unknown) => {
    ...
    if (activeTabId) {
      await supabase
        .from('math_problems')
        .update({ data: { elements, appState }, updated_at: new Date().toISOString() })
        .eq('id', activeTabId)
    }
  }
  ...
  const saveVersion = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({ problem_id: tab.id, data: tab.data })
    ...
    const newVersion = { id: data.id, created_at: data.created_at, data: data.data }
  ...
  const restoreVersion = async (versionId: string) => {
    ...
    const newTabs = tabs.map((t) =>
      t.id === tab.id ? { ...t, data: version.data } : t
    )
    setTabs(newTabs)
    await supabase
      .from('math_problems')
