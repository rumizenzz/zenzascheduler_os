...
export function MathNotebookModule() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (data && data.length > 0) {
        ...
        const problemIds = formatted.map((p: any) => p.id)
        const { data: textData, error: textError } = await supabase
          .from('math_text_history')
          .select('problem_id, element_id, text, edited_at')
          .in('problem_id', problemIds)

        if (textError) {
          console.error('Failed to load text history:', textError)
        } else if (textData) {
          const histories: Record<string, { text: string; editedAt: string }[]> = {}
          textData.forEach((h: any) => {
            if (!histories[h.element_id]) histories[h.element_id] = []
            histories[h.element_id].push({ text: h.text, editedAt: h.edited_at })
          })
  ...
  const saveEdit = async () => {
    ...
    setTextHistories((prev) => ({
      ...
      [editing.id]: [
        ...(prev[editing.id] || []),
        { text: editing.original, editedAt },
      ],
    }))
    if (activeTabId) {
      ...
      if (error) {
        ...
        toast.error('Failed to save text history')
      }
    }
    setEditing(null)
  }
