...
export function ReadVerse() {
  ...
  useEffect(() => {
    if (view === 'library' && source === 'bible') {
      fetchBibleChapter()
    }
  }, [view, source, bibleBook, chapter])
    ...
    const verses =
      source === 'bom'
        ...
        : bibleChapterVerses.map((v) => ({
            reference: `${bibleBook} ${chapter}:${v.verse}`,
            text: v.text
          }))

    const bookIndex = bibleBooks.indexOf(bibleBook)
          ...
          {source === 'bible' && (
            <div className="flex flex-wrap gap-2 items-center">
              <button
                onClick={() => {
                  const prev =
                    (bookIndex - 1 + bibleBooks.length) % bibleBooks.length
                  setBibleBook(bibleBooks[prev])
                  setChapter(1)
                  setVerseNumber(1)
                }}
              ...
              <button
                onClick={() => {
                  const next = (bookIndex + 1) % bibleBooks.length
                  setBibleBook(bibleBooks[next])
                  setChapter(1)
                  setVerseNumber(1)
                }}
                className="btn-dreamy"
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    if (source === 'bom') {
                      ...
                      const verse = bibleChapterVerses[i]
