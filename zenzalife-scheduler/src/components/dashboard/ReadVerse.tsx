  ...
  const fetchBomChapter = async (book = bomBook, chap = chapter) => {
    ...
    try {
      ...
      for (let start = 1; start <= 200; start += 50) {
        const batch = Array.from({ length: 50 }, (_, i) => start + i).map(
          (v) =>
            fetch(`https://book-of-mormon-api.vercel.app/${slug}/${chap}/${v}`)
              .then((res) => res.json())
              .then((data) =>
                data.text
                  ? { reference: data.reference, text: data.text.trim() }
                  : null
              )
              .catch(() => null)
        )
        const results = (await Promise.all(batch)).filter(Boolean) as {
          reference: string
          text: string
        }[]
        verses.push(...results)
