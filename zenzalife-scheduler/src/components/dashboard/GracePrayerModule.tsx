  ...
  const handleStop = async () => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        if (photoFile) {
          const photoBase = await new Promise<string>((resolve, reject) => {
            const r = new FileReader()
            r.onloadend = () => resolve(r.result as string)
            r.onerror = () => reject(new Error('Photo read failed'))
            r.readAsDataURL(photoFile)
          })
          const imgName = `meal-${mealTime}-${Date.now()}-${photoFile.name}`
          const imgRes = await supabase.functions.invoke('audio-upload', {
            body: { audioData: photoBase, fileName: imgName, userId: user!.id }
          })
          if (imgRes.error || !imgRes.data?.publicUrl) {
            throw imgRes.error || new Error('Image upload failed')
          }
          photoUrl = imgRes.data.publicUrl as string
        }
