...
export function GracePrayerModule() {
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      const msg = message.startsWith("Upload failed")
        ? message
    ...
    <div className="space-y-4">
      ...
      {startTime && recording && (
        <p className="text-sm text-gray-500">
          Started at {dayjs(startTime).format("h:mm:ss A")} - Recording {duration}s
        </p>
      )}
      {!recording && lastDuration !== null && (
        <p className="text-sm text-gray-500">Last recording: {lastDuration}s</p>
      )}
