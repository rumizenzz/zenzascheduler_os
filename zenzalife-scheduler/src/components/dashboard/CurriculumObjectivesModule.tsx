...
export function CurriculumObjectivesModule() {
  ...
  const loadCurriculums = async (retry = false) => {
    if (!user) return
    ...
    if (error) {
      if (
        !retry &&
        error.message.includes('relation "public.curriculums" does not exist')
      ) {
        try {
          await supabase.functions.invoke('ensure-curriculum-schema')
          return await loadCurriculums(true)
        } catch (err) {
          console.error('Failed to ensure curriculum schema', err)
        }
      }
  ...
  const loadObjectives = async (curriculumId: string, retry = false) => {
    if (!user) return
    setLoading(true)
    const { data, error } = await supabase
    ...
    if (error) {
      if (
        !retry &&
        error.message.includes('relation "public.curriculum_objectives" does not exist')
      ) {
        try {
          await supabase.functions.invoke('ensure-curriculum-schema')
          return await loadObjectives(curriculumId, true)
        } catch (err) {
          console.error('Failed to ensure curriculum schema', err)
        }
      }
