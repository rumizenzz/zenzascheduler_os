          ...
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full space-y-4 text-center">
            {status === 'subscribed' ? (
              <>
                ...
                <p className="text-gray-600 font-light">You can unsubscribe or stay subscribed below.</p>
                <div className="flex gap-4 justify-center">
                  <button onClick={unsubscribe} className="btn-dreamy flex-1" disabled={loading}>
                    {loading ? 'Saving...' : 'Unsubscribe'}
                  </button>
                  <button onClick={close} className="btn-dreamy-primary flex-1">Close</button>
                </div>
              </>
            ) : (
              <>
