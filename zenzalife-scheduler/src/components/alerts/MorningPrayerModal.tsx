...
interface MorningPrayerModalProps {
  isOpen: boolean
  onClose: () => void
}

export function MorningPrayerModal({ isOpen, onClose }: MorningPrayerModalProps) {
  ...
  const handleDone = async () => {
    ...
    try {
      ...
      const filePath = `prayers/${user.id}/morning-${Date.now()}.webm`
      ...
      toast.success('Morning prayer saved')
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      <div className="bg-white rounded-xl shadow-xl p-6 space-y-4 max-w-sm w-full text-center">
        <h2 className="text-2xl font-bold text-blue-700">Morning Prayer</h2>
        {!recording && !audioUrl && (
