...
export function TaskModal({ isOpen, onClose, onSave, onDelete, task, initialDate }: TaskModalProps) {
  const [formData, setFormData] = useState({
    ...
    repeat_pattern: 'none',
    alarm: false,
    custom_sound_path: localStorage.getItem('defaultAlarmSound') || '/alarms/classic-alarm-995.mp3',
    visibility: 'private',
    completed: false
  })
  ...
  useEffect(() => {
    if (task) {
      setFormData({
        ...
        custom_sound_path: task.custom_sound_path || localStorage.getItem('defaultAlarmSound') || '/alarms/classic-alarm-995.mp3',
              ...
              {formData.alarm && (
                <div className="w-full space-y-1">
                  ...
                  <select
                    ...
                    className="input-dreamy w-full"
                  >
                    {[...builtinAlarms, ...JSON.parse(localStorage.getItem('customAlarmSounds') || '[]')].map(url => (
